...
from data_aug.contrastive_learning_dataset import ContrastiveLearningDataset, LocalAggregationDataset
from models.resnet_simclr import ResNetSimCLR
from simclr import SimCLR
from localagg import LocalAggregation
from utils import *
import copy
import time
...
def run_la(args, model, log_dir):
    ...
    log_dir = os.path.join(log_dir, 'la')
    os.makedirs(log_dir)
    #  It’s a no-op if the 'gpu_index' argument is a negative integer or None.
    with torch.cuda.device(args.gpu_index):
        la = LocalAggregation(model=model, args=args, dataset=train_dataset, log_dir=log_dir)
        la.train(train_loader)
